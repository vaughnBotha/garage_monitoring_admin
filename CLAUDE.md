...
Target platforms: macOS, Linux, and web desktop (mouse + right-click), not touch. No platform-specific APIs or plugins are used, so it should build on Linux without changes — verify with a real `flutter run -d linux` there, since this can't be cross-compiled or tested from macOS.
...
- `FloorData` (→ `ParkingLevel`) has a background floor-plan image (`assets/floorplans/*.png`, rasterized once from the source PDFs — no PDF-rendering plugin, to keep Linux compatibility) and a list of `ZoneData`.
